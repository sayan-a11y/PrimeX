    ...
    <div className="flex items-center gap-6">
      <div className="relative w-32 h-32 shrink-0">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-lg font-bold primex-gradient-text">${(total / 1000).toFixed(1)}k</span>
      ...
      <div className="space-y-2">
        {segments.map((seg, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium ml-auto">${seg.value.toLocaleString()}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function CreatorDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 lg:p-6 bg-mesh min-h-screen relative">
      {/* Decorative orbs */}
      <div className="orb-primex absolute top-10 right-10 opacity-20 pointer-events-none" />
      <div className="orb-primex-md absolute bottom-20 left-5 opacity-15 pointer-events-none" />
      ...
      <div className="relative z-10 mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <span className="tag-primex text-xs">Pro Creator</span>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="glass-card w-full h-11 rounded-xl p-1 mb-6 relative z-10">
          ...
          <TabsTrigger value="content" className="rounded-lg flex-1 data-[state=active]:primex-gradient data-[state=active]:text-white gap-1.5">
            <Play className="w-4 h-4" /> Content
          </TabsTrigger>
          <TabsTrigger value="audience" className="rounded-lg flex-1 data-[state=active]:primex-gradient data-[state=active]:text-white gap-1.5">
            <Users className="w-4 h-4" /> Audience
          </TabsTrigger>
          <TabsTrigger value="schedule" className="rounded-lg flex-1 data-[state=active]:primex-gradient data-[state=active]:text-white gap-1.5">
        ...
        <TabsContent value="overview">
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-6 rounded-2xl">
                <div className="flex items-center gap-2 mb-3">
                  <DollarSign className="w-5 h-5 text-green-400" />
                  <span className="text-sm text-muted-foreground">Total Earnings</span>
                </div>
                <p className="text-3xl font-bold primex-gradient-text count-up">${earningsData.total.toLocaleString()}</p>
              ...
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-6 rounded-2xl">
                ...
                <p className="text-3xl font-bold primex-gradient-text count-up">${earningsData.thisMonth.toLocaleString()}</p>
              ...
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-6 rounded-2xl">
                ...
                <p className="text-3xl font-bold primex-gradient-text count-up">
                ...
                {earningsData.monthly.map((m, i) => {
                  ...
                  return (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-2">
                      <span className="text-xs text-muted-foreground">${(m.amount / 1000).toFixed(1)}k</span>
            ...
            <motion.div variants={staggerItem} className="glass-card-premium p-6 rounded-2xl">
              ...
              <div className="space-y-2">
                {earningsData.payouts.map((p) => (
                  <div key={p.id} className="glass-card hover-lift p-4 rounded-xl flex items-center gap-3">
                    ...
                    <Badge className="bg-green-500/20 text-green-400 text-xs">{p.status}</Badge>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <motion.div key={m.label} variants={staggerItem} className="glass-card-premium card-shine hover-lift p-4 rounded-2xl text-center">
                  <m.icon className={`w-6 h-6 ${m.color} mx-auto mb-2`} />
                  <p className="text-2xl font-bold primex-gradient-text count-up">{m.value}</p>
            ...
            <motion.div variants={staggerItem}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {topVideos.map((video, idx) => (
                  <motion.div key={video.id} variants={staggerItem} className="glass-card-premium card-shine hover-lift rounded-2xl overflow-hidden gradient-border-primex">
                    {/* Thumbnail */}
                    ...
                    <div className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs">{(video.likes / 1000).toFixed(1)}K</span>
                        </div>
                        <div className="flex items-center gap-1">
            ...
            <motion.div variants={staggerItem}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { type: 'Videos', count: 48, views: '95.2K', color: 'from-primex/20 to-primex/5' },
                  { type: 'Reels', count: 124, views: '47.3K', color: 'from-primex-secondary/20 to-primex-secondary/5' },
                ].map((c) => (
                  <div key={c.type} className="glass-card-premium hover-lift p-5 rounded-2xl gradient-border-primex">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{c.count} published</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Eye className="w-4 h-4 text-muted-foreground" />
                      <span className="text-lg font-bold primex-gradient-text">{c.views}</span>
        ...
        <TabsContent value="audience">
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-6">
            {/* Followers Stats */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-5 rounded-2xl">
                <Users className="w-5 h-5 text-primex mb-2" />
                <p className="text-2xl font-bold primex-gradient-text count-up">{(audienceData.totalFollowers / 1000).toFixed(1)}K</p>
                <p className="text-xs text-muted-foreground">Total Followers</p>
              </motion.div>
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-5 rounded-2xl">
                <div className="flex items-center gap-2 mb-2">
                  <Zap className="w-5 h-5 text-green-400" />
                  <span className="badge-pulse text-[10px]">Live</span>
                </div>
                <p className="text-2xl font-bold primex-gradient-text count-up">{(audienceData.activeFollowers / 1000).toFixed(1)}K</p>
                <p className="text-xs text-muted-foreground">Active Followers</p>
              </motion.div>
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-5 rounded-2xl">
                <TrendingUp className="w-5 h-5 text-primex-secondary mb-2" />
                <p className="text-2xl font-bold primex-gradient-text count-up">+{audienceData.followerGrowth[0].gained}</p>
                <p className="text-xs text-muted-foreground">This Week</p>
              </motion.div>
              <motion.div variants={staggerItem} className="glass-card-premium card-shine hover-lift p-5 rounded-2xl">
                <Award className="w-5 h-5 text-yellow-400 mb-2" />
                <p className="text-2xl font-bold primex-gradient-text count-up">Top 5%</p>
                  ...
                  {audienceData.ageGroups.map((ag) => (
                    <div key={ag.range}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-medium primex-gradient-text">{ag.pct}%</span>
                      </div>
                    ...
                    <div key={loc.country} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-sm font-medium">{loc.pct}%</span>
                        </div>
                        <div className="progress-bar">
            ...
            <motion.div variants={staggerItem} className="glass-card-premium p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-3 gap-4">
                {audienceData.devices.map((d) => (
                  <div key={d.type} className="text-center p-4 glass-card rounded-xl hover-lift">
                    <d.icon className="w-6 h-6 text-primex mx-auto mb-2" />
                    <p className="text-lg font-bold primex-gradient-text">{d.pct}%</p>
            ...
            <motion.div variants={staggerItem} className="glass-card-premium p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {audienceData.followerGrowth.map((fg, i) => (
                  <div key={fg.period} className="glass-card p-4 rounded-xl text-center hover-lift">
            ...
            <motion.div variants={staggerItem} className="glass-card-premium p-6 rounded-2xl">
              ...
              <div className="space-y-2">
                {audienceData.topFans.map((fan, i) => (
                  <div key={fan.username} className="glass-card hover-lift p-3 rounded-xl flex items-center gap-3">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium primex-gradient-text">${fan.contributions}</p>
                ...
                {scheduleData.calendar.days.map((day) => (
                  <div
                    ...
                    className={`aspect-square rounded-lg flex flex-col items-center justify-center text-sm transition-all cursor-pointer ${
                      day.isToday
                        ? 'primex-gradient text-white font-bold'
                        : day.hasContent
                          ? 'bg-primex/10 text-primex hover:bg-primex/20'
              ...
              <div className="space-y-3">
                {scheduleData.queue.map((item, idx) => (
                  <motion.div
                    ...
                    className="glass-card hover-lift p-4 rounded-xl flex items-center gap-3"
                  >
